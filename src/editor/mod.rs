...
use bevy_egui::{EguiGlobalSettings, EguiPrimaryContextPass, PrimaryEguiContext, egui};
...
use bevy_pancam::{PanCam, PanCamClampBounds};
...
pub(super) fn plugin(app: &mut App) {
    app
        ...
        .init_state::<EditorToggle>()
        .add_plugins(bevy_egui::EguiPlugin::default())
        .add_plugins(DefaultInspectorConfigPlugin)
        .insert_resource(UiState::new())
        .add_systems(
            EguiPrimaryContextPass,
            show_ui_system.run_if(in_state(EditorToggle::Enabled)),
        )
        .add_systems(PostUpdate, set_camera_viewport.after(show_ui_system))
        .add_systems(
            Update,
            draw_mesh_intersections.run_if(in_state(EditorToggle::Enabled)),
        )
        //.add_systems(PostUpdate, handle_pick_events)
        .register_type::<Option<Handle<Image>>>()
        .register_type::<AlphaMode>()
        .add_systems(OnEnter(Screen::Editor), enable_editor)
        .add_systems(OnExit(Screen::Gameplay), disable_editor);
}
...
fn enable_editor(
    ...
    mut editor_toggle: ResMut<NextState<EditorToggle>>,
) {
    editor_toggle.set(EditorToggle::Enabled);
...
fn disable_editor(mut editor_toggle: ResMut<NextState<EditorToggle>>) {
    editor_toggle.set(EditorToggle::Disabled);
}
